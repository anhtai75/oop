...
public:
    ...
    void tim_tai_lieu_dien_tu() const {
        ...
        if (!found) {
            cout << "⚠️ Không tìm thấy tài liệu [" << ten_tim << "] loại [" << loai_dt_tim << "]." << endl;
        }
    }
    ...
    void tra_tai_lieu(const string& ma_bd, const string& ma_tl) {
        ...
        if (gd_muon_gan_nhat) {
            ...
            if (thoi_gian_hien_tai > thoi_gian_dk) {
                ...
                cout << "⚠️ CHÚ Ý: Trả trễ " << so_ngay_tre << " ngày." << endl;
            }
        }
    ...
    void thong_ke_so_luot_doc() const {
        ...
        cout << "▶️ Chi tiết theo Mã tài liệu:" << endl;
    ...
    void de_xuat_tai_lieu(const string& ma_bd) const {
        ...
        if (favorite_genres.empty()) {
            cout << "⚠️ Lịch sử giao dịch trống. Không thể đề xuất." << endl;
            return;
        }
        ...
        cout << "▶️ Thể loại yêu thích nhất: [" << top_genre << "] (Tương tác: " << max_count << " lần)" << endl;
        cout << "▶️ Các tài liệu liên quan (chưa mượn):" << endl;
